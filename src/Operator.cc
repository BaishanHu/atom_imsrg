...
{
  //cout << "Inside simple constructor, setting OneBodyChannels." << endl;
  SetUpOneBodyChannels();
  //cout << "Set One body, checking p_rank and allocating for 3body." << endl;
  if (particle_rank >=3) ThreeBody.Allocate();
  //cout << "Three body allocated (or not), adding to N_Operators, and exiting." << endl;
  profiler.counter["N_Operators"] ++;
}
      ...
      for (index_t a=0;a<norbits;++a)
      {
         ...
         for ( auto& b : opNO.OneBodyChannels.at({oa.l,oa.j2,oa.tz2}) ) 
         {
            ...
            for (auto& h : modelspace->holes)  // C++11 syntax
            {
              ...
              if (opNO.rank_J==0)
              {
                 opNO.OneBody(a,b) += (2*J_ket+1.0)/(2*ja+1) * oh.occ * TwoBody.GetTBME(ch_bra,ch_ket,a,h,b,h);
              }
              else
              {
                 double jh = oh.j2/2.0;
                 opNO.OneBody(a,b) += sqrt((2*J_bra+1.0)*(2*J_ket+1.0)) * oh.occ *modelspace->phase(ja+jh+J_ket+opNO.rank_J)
